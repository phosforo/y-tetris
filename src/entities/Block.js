...
export default class Block extends Container {
    ...
    get nextRotateState() {
        const nextRotationState = this.currentState + 1 >= BlockShapeConst.SHAPE_SIZE  ? 0 : this.currentState + 1;
        return this.shape.states[nextRotationState];
    }

    rotate() {
        this.currentState += this.currentState >= BlockShapeConst.SHAPE_SIZE ? 0 : 1;
    }
